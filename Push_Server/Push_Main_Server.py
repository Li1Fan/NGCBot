...
from Util.util_db import TimingMsgDB
...
class Push_Main_Server:
    def __init__(self, wcf, rms=None):
        self.wcf = wcf
        self.rms = rms
    ...
    # 清空AI最大调用次数记录表
    def clear_ai_max(self):
        OutPut.outPut(f'[*]: 定时AI最大调用次数表格清空中... ...')
        self.rms.ai_max_call = {}
        OutPut.outPut(f'[+]: 定时AI最大调用次数表格清空成功！！！')
    ...
    def run(self):
        ...
        schedule.every().day.at('03:00').do(self.clear_ai_max)
...
class TimingMsg:
    ...
    def send_at_msg(self, roomid, wxid, content):
        at_msg = f"@{self.rms.get_alias_in_chatroom(roomid=roomid, wxid=wxid)}\n{content}"
