...
class Db_Point_Server:
    ...
    # 查询积分前十名
    def query_top(self, room_id):
        conn, curser = self.open_db()
        query_top_sql = f'''SELECT wx_id, point FROM points WHERE room_id = '{room_id}' ORDER BY point DESC LIMIT 10;'''
        curser.execute(query_top_sql)
        data = curser.fetchall()
        return data

    # 查询积分倒数前十名
    def query_bottom(self, room_id):
        conn, curser = self.open_db()
        query_bottom_sql = f'''SELECT wx_id, point FROM points WHERE room_id = '{room_id}' ORDER BY point ASC LIMIT 10;'''
        curser.execute(query_bottom_sql)
        data = curser.fetchall()
        return data
    ...
    # 归零积分
    def zero_point(self, wx_id, wx_name, room_id, room_name):
        if self.judge_user(wx_id=wx_id, room_id=room_id):
            conn, curser = self.open_db()
            zero_point_sql = f'''UPDATE points SET point=0 WHERE wx_id = '{wx_id}' AND room_id = '{room_id}';'''
            curser.execute(zero_point_sql)
            conn.commit()
            msg = f'基于您的表现，尊贵的管理员给你小子归零了积分，请你好好表现\n当前可用积分: {self.query_point(wx_id=wx_id, wx_name=wx_name, room_id=room_id, room_name=room_name)}'
        else:
            self.add_user(wx_id=wx_id, wx_name=wx_name, room_id=room_id, room_name=room_name)
            msg = self.zero_point(wx_id=wx_id, wx_name=wx_name, room_id=room_id, room_name=room_name)
        return msg
